...
In September 2025, I had privilege to join one of the most famous competitions in Product Design field of Vietnam: Lollypop Designathon. This is a unique event where teams match against each other in a 24-hour race to deliver a solution matched with the given subject of the game.

Our team was given a very interesting challenge: using AI responsibly and utilizing computer vision feature to solve a particular problem of Vietnam's traffics.
## The Challenge

For a normal rider, navigating on the roads they often travel is usually not a problem. However, what if they get to a particular unfamiliar route with many strange road signs they haven't known before, or when there are new rules on the roads they often use? They will be often stuck in a difficult scenario when they don't actually know which rules to follow, which creates a fear that they will be get fined heavily if they did a misstep and traffic police caught their action. 

![A crowded street filled with motorbikes in a busy Vietnamese city, capturing urban chaos.](images/vietnam-heavy-traffics.jpg "<strong>The challenge in navigating traffics in Vietnam</strong>. Every day on the road feels like an uphill battle for many people")
{.full}

Google Maps - a trusted navigation app used by many here, actually does not support road signs alert. At best, it only does what it does best: showing you how to navigate to destination, not teach you how to ride safely when you are on the road. 

The problem would be worsen if we do not actually address the core issue that many motorcycle users are having: they don't actually know and understand many of the nuances or hidden rules when participating in the traffics. 

!["How can we get people to easily learn traffic laws and gets alerted to their traffic violations on the road"](images/problem_statement.png)
...
![A collection of news articles focusing on Vietnam's traffic issues, including enforcement statistics and public awareness initiatives](images/desk_research.png "<strong>Our desk research from news articles</strong>. A problem that still haunting many people when travelling on the road and for officials to make the road safer for everyone.")
...
![Three problem–opportunity–outcome pairs from user research](images/opportunities.png "<strong>The app's opportunities</strong>. Derived from survey findings across 43–51 respondents.")
...
![Papers detailing about product specifications](images/drafts.png "<strong>Sketching ideas</strong>. I explored how we might deliver our solution to our main audience, and what information content we should present to them.")
...
![The map alert feature which allows user to track and analyze the mistakes they made when on traffic](images/tracking_iteration.png "<strong>The tracking feature wireframe</strong>. A prominent map showing where you made the mistake and a list of most recent mistakes you did based on the map.")
...
![The Home page screen of the app](images/home_screen.png "<strong>The Home page of our app</strong>. The screen includes news popup come from a toast notification, the latest report from recent ride and the behavior score on user's license.")
...
![Two screens, one displaying the News screen and other displaying the Ranking screen](images/news_and_challenge_screen.png "<strong>The News and Ranking screen</strong>. While the News page displaying selected news for user, the Ranking screen offers engagement through safety challenges with gamification purpose.")
...
![Diagram showing a three-step process: a helmet-mounted dashcam records the riding journey, pushes data to the app (shown as phone notification) and AI analyzes it](images/dash-cam-ai.png "The camera on user's helmet continously records the journey. The footage is sent to the app in real time, where AI analyzes the route and flags risky behaviors - in this example, detected 3 errors when on the trip")
